...
- Optionally renders a visualization of the predicted board.
"""
...
CONF_THRESHOLD = 0.85      # Must be 85% sure to not mark as OOD
ENTROPY_THRESHOLD = 0.6    # Lower value means the model is more certain
LAPLACIAN_THRESHOLD = 30 
CANNY_THRESHOLD = 110
# NEIGHBOR_OOD_LIMIT removed
OVERLAP_PERCENT = 0.7

# Maps internal class names to the required integer encoding.
MAPPING_TO_API = {
...
# Global model used by `predict_board()`.
...
def is_visually_valid(pil_tile):
    """
    Verifies geometric integrity by detecting at least 2 of 4 corners.
    If 2 corners are found near the tile boundaries, it's considered valid.
    """
    try:
        ...
        zones = [
            ...
            (0, margin, w - margin, w),       # Top-Right
            (h - margin, h, 0, margin),       # Bottom-Left
            (h - margin, h, w - margin, w)    # Bottom-Right
        ]
...
def predict_board(image: np.ndarray, debug_compare: bool = False) -> torch.Tensor:
    ...
    if debug_compare:
        pred_board_img = visual_board_from_matrix(final_output, out_path=None)
        canvas = Image.new('RGB', (1600, 800))
        canvas.paste(img_pil, (0, 0))
        canvas.paste(pred_board_img.resize((800, 800)), (800, 0))
...
def visual_board_from_matrix(matrix_tensor, square_size=80, border=20, light_color=(240, 217, 181), dark_color=(181, 136, 99), piece_scale=0.8, out_path=None):
...
if __name__ == "__main__":
    ...
    _inference_model = load_model(model_path, device)
    ...
    if os.path.exists(input_folder):
        ...
        for img_name in img_files:
            ...
            default_debug_path = os.path.join(output_base_dir, "debug_comparison.png")
            new_debug_path = os.path.join(output_base_dir, f"debug_{os.path.splitext(img_name)[0]}.png")
            if os.path.exists(default_debug_path):
                if os.path.exists(new_debug_path):
                    os.remove(new_debug_path)
                os.rename(default_debug_path, new_debug_path)
            
            final_visual_path = os.path.join(output_base_dir, f"board_{os.path.splitext(img_name)[0]}.png")
            visual_board_from_matrix(board_tensor, out_path=final_visual_path)