...
# --- הגדרות ומחלקות ---
# וודאי שהסדר כאן תואם בדיוק לסדר התיקיות ב-final_dataset/train
...
# מיפוי משם המחלקה חזרה לאות FEN
CLASS_TO_FEN = {
...
def get_fen_from_matrix(matrix):
    """ הופך מטריצת תווים למחרוזת FEN תקנית """
    fen_rows = []
    for row in matrix:
        fen_row = ""
        empty_count = 0
...
def process_image(image_path, model, device, overlap_percent=0.7):
    # טעינת התמונה לציור (OpenCV) ולעיבוד (PIL)
    img_cv = cv2.imread(image_path)
    img_cv = cv2.resize(img_cv, (800, 800)) # ננרמל לגודל קבוע
    ...
    for r in range(8):
        current_row = []
        for c in range(8):
            # חישוב חיתוך עם Overlap בדיוק כמו באימון
            ...
            # --- דרישת המייל: סימון איקסים אדומים בביטחון נמוך ---
            if conf.item() < 0.75: # סף ביטחון (ניתן לכיוונון)
...
if __name__ == "__main__":
    ...
    test_img = 'path_to_test_image.jpg' # התמונה שהמרצה יבדוק
